from tkinter import Tk, Button, Listbox, PhotoImage, Frame, Menu, filedialog, END, ACTIVE, ANCHOR, Label, GROOVE, E, BOTTOM, X, HORIZONTAL
import pygame
import time
...
# !
# * ADDED Song position slider   https://www.youtube.com/watch?v=gC8dRDqWw6I&list=PLXLYwvNGGPoUzjKiGvuXm8qeiOYMnFria&index=8
# ?
# TODO Volume control
...
global paused, playing, stopped, song_length
paused = False
playing = False
stoppped = True
...
def play_time() -> None:
    ...
    if int(my_slider.get()) == int(song_length):
        ...
        # Update slider position's max value
        my_slider.config(to=slider_position, value=int(current_time))
        status_bar.config(text=f'{converted_current_time}/{converted_song_length}  ')
    else:
        # slider has moved
        # Update slider position's max value
        time_increament = int(my_slider.get()) + 1
        my_slider.config(to=slider_position, value=time_increament)
        converted_current_time = time.strftime('%M:%S', time.gmtime(current_time))
        status_bar.config(text=f'{converted_current_time}/{converted_song_length}  ')
        
    #slider_label.config(text=f"Slider position: {int(my_slider.get())}\nSong position: {int(current_time)}")
...
def previous_song() -> None:
    #find next song
    current_file = song_box.curselection()
    next_file = current_file[0] - 1
    song = song_box.get(next_file)
    song = f"{MUSIC_FOLDER}{song}.mp3"
    ...
    song_box.activate(next_file)  # sets active bar to next song
    song_box.selection_set(next_file, last=None)  # draw the bar
...
def next_song() -> None:
    ...
    song_box.activate(next_file)
    song_box.selection_set(next_file, last=None)
...
# Create Status bar
status_bar = Label(root, text="", bd=1, relief=GROOVE, anchor=E, background="green")
